...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      <aside 
        className={`bg-white dark:bg-[#18191D] h-screen fixed left-0 top-0 z-50 transition-all duration-300 border-r border-[#E8E5DF] dark:border-[#2B2D34] flex flex-col justify-between ${
          mobileDrawerOpen 
            ? 'translate-x-0 w-72 shadow-2xl' 
            : '-translate-x-full md:translate-x-0 ' + (collapsed ? 'w-20' : 'w-64')
        }`}
      >
        <div>
          ...
          {!isInstalled && (!collapsed || mobileDrawerOpen) && (
            <div className="mx-4 mt-4 p-3 bg-[#FAF8F5] dark:bg-[#121316] border border-[#E8E5DF] dark:border-[#2B2D34] rounded-sm space-y-2">
